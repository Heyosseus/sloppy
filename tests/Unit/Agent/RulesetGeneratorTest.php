...
it('writes a Boost guideline that Blade passes through untouched', function (): void {
    $guideline = generatorFor()->generate(RulesetFormat::Boost);
    $lines = explode(PHP_EOL, rtrim($guideline));

    expect($lines[0])->toBe(RulesetGenerator::BOOST_MARKER)
        ->and($lines[1])->toBe('@verbatim')
        ->and($lines[2])->toBe('# Sloppy: code rules for this repository')
        ->and(end($lines))->toBe('@endverbatim')
        ->and($guideline)->toContain('`sloppy rules --format=boost`')
        ->and($guideline)->toContain('### SL101 God Method')
        ->and(mb_substr_count($guideline, '@verbatim'))->toBe(1);
});
